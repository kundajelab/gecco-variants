...
n_tss=62188
h3k27ac_tss=read.table("tss_to_prox_enh_counts.reg2map.p10.txt",header=FALSE,sep='\t')
...
h3k27ac_genes=read.table("gene_to_prox_enh_counts.reg2map.p10.txt",header=FALSE,sep='\t')
p10=ggplot(data=h3k27ac_genes,aes(x=h3k27ac_genes$V1,y=h3k27ac_genes$V2,fill=h3k27ac_tss$V1))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=colors,name="Dataset")+
  ...
  theme_bw(20)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
multiplot(p1,p10,cols=2)

h3k27ac_enh=read.table("prox_enh_counts.reg2map.p10.txt",header=FALSE,sep='\t')
